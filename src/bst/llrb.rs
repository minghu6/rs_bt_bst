...
impl<'a, K: CollKey + 'a, V: 'a> PhB4Node<K, V> {
    ...
    fn item_iter(&'a self) -> Box<dyn Iterator<Item = *mut LLRBNode<K, V>> + 'a> {
        let mut i = -1i32;
    ...
    fn children_iter(&'a self) -> Box<dyn Iterator<Item = *mut LLRBNode<K, V>> + 'a> {
        let mut i = -1i32;
    ...
    unsafe fn pop_item(&mut self, t: &mut LLRB<K, V>, dir: Either<(), ()>) -> *mut LLRBNode<K, V> {
        match (self.node_size(), dir) {
            (3, Either::Left(())) => {
                let child = (*self.centre).left;

                (*self.centre).assign_left(null_mut::<LLRBNode<K, V>>());
                self.centre = t.rotate(self.centre, Either::Left(())) as *mut LLRBNode<K, V>;
...
impl<'a, K: CollKey + 'a, V: 'a> PhB3Node<K, V> {
    ...
    unsafe fn pop_item(&mut self, t: &mut LLRB<K, V>, dir: Either<(), ()>) -> *mut LLRBNode<K, V> {
        match (self.node_size(), dir) {
            ...
            (2, Either::Right(())) => {
                let root = t.rotate(self.centre, Either::Right(())) as *mut LLRBNode<K, V>;
...
impl<'a, K: CollKey + 'a, V: 'a> LLRBNode<K, V> {
    ...
    pub fn self_validate(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        unsafe {
            ...
            // unbalanced
            assert!(!(is_red(self.left) && is_red((*self.left).left)));
...
impl<'a, K: CollKey + 'a, V: 'a> LLRB<K, V> {
    ...
    unsafe fn promote(&mut self, x: *mut LLRBNode<K, V>) {
        ...
        let mut x_dir = (*x).dir();
        let x_sibling = (*x_paren).child_bst(x_dir.reverse()) as *mut LLRBNode<K, V>;
        ...
        let u_paren = (*u).paren;
        let u_dir = (*u).dir();
        let u_sibling = (*u_paren).child_bst(u_dir.reverse()) as *mut LLRBNode<K, V>;
    ...
    #[allow(unused)]
    unsafe fn unpromote_b3(&mut self, mut paren_b3: PhB3Node<K, V>, leaf_idx: usize) {
    ...
    unsafe fn fix_spec_2_node_up(&mut self, x: *mut LLRBNode<K, V>) {
        ...
        if is_nonnil_black((*sibling).left) && is_nonnil_black((*sibling).right) {
    ...
    unsafe fn fixup(&mut self, mut x: *mut LLRBNode<K, V>) -> *mut LLRBNode<K, V> {
        debug_assert!(!x.is_null());
    ...
    unsafe fn remove_min_(&mut self, mut x: *mut LLRBNode<K, V>) -> *mut LLRBNode<K, V> {
        if (*x).left.is_null() {
            return null_mut();
        }
    ...
    ///   (b)
    ///```
    unsafe fn move_red_left(&mut self, mut x: *mut LLRBNode<K, V>) -> *mut LLRBNode<K, V> {
        (*x).color_flip();
    ...
    unsafe fn move_red_right(&mut self, mut x: *mut LLRBNode<K, V>) -> *mut LLRBNode<K, V> {
        (*x).color_flip();
    ...
    unsafe fn remove_(
        ...
        if key < (*x).key_bst() {
            ...
            if !x_lf.is_null() {
                ...
                (*x).connect_left(self.remove_((*x).left, key, res));
            }
        } else {
            if is_red((*x).left) {
                x = self.rotate(x, Either::Right(())) as *mut LLRBNode<K, V>;
            ...
            if !(*x).right.is_null() {
                ...
                if key == (*x).key_bst() {
                    ...
                    (*x).connect_right(self.remove_min_((*x).right))
                } else {
                    (*x).connect_right(self.remove_((*x).right, key, res))
                }
            }
        }
