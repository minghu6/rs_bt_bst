...
impl<'a, K: CollKey + 'a, V: 'a> AA<K, V> {
    ...
    unsafe fn insert_at(
        ...
        key: K,
        value: V,
    ) -> Result<*mut AANode<K, V>, ()> {
        if t.is_null() {
            t = AANode::new(key, value);
        } else if key < *(*t).key {
            (*t).connect_left(self.insert_at((*t).left, key, value)?)
        } else if key > *(*t).key {
            (*t).connect_right(self.insert_at((*t).right, key, value)?)
        } else {
            return Err(());
        }
    ...
    unsafe fn split(&mut self, t: *mut AANode<K, V>) -> *mut AANode<K, V> {
        ...
        if (*(*(*t).right).right).level == (*t).level {
            let new_t = self.rotate(t, Either::Left(())) as *mut AANode<K, V>;
    ...
    unsafe fn remove_at(
        ...
        if key < (*t).key_bst() {
            (*t).connect_left(self.remove_at((*t).left, key, res));
        } else if key > (*t).key_bst() {
            (*t).connect_right(self.remove_at((*t).right, key, res));
        } else {
            if (*t).is_leaf() {
                res.push((*t).value);
                return (*t).left; // null
                ...
                (*t).connect_right(self.remove_at((*t).right, (*succ).key_bst(), res));
                ...
                (*t).connect_left(self.remove_at((*t).left, (*prec).key_bst(), res));
        ...
        t = self.skew(t);
        (*t).connect_right(self.skew((*t).right));

        if !(*t).right.is_null() {
            let t_right = (*t).right;

            (*t_right).connect_right(self.skew((*t_right).right));
        }

        t = self.split(t);
        (*t).connect_right(self.split((*t).right));
        ...
        if !self.root.is_null() {
            unsafe {
                ...
                for node in self.nodes_iter() {
                    ...
                    if (*x).level > 1 {
                        assert!(!(*x).left.is_null() && !(*x).right.is_null())
                    }
                }
            }
        }
